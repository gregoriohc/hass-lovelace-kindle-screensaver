...
| `RENDERING_DELAY`         | `0`                                   | no       | yes      | Optional extra settle buffer (ms) applied *after* the dashboard is detected as ready. No longer the primary readiness mechanism — leave at `0` unless a custom card keeps painting after stabilising. |
| `RENDERING_WAIT_FOR_CONTENT` | `true`                             | no       | yes      | Wait for the Lovelace dashboard to actually finish rendering (content present, no loading spinners, images loaded, DOM stable) before the screenshot. Set to `false` to fall back to the old fixed-delay-only behaviour. |
| `RENDERING_STABILITY_TIMEOUT` | `30000`                          | no       | yes      | Max time (ms) to wait for the dashboard content to become ready/stable. If content appears but never stabilises, the screenshot is taken anyway; if content never appears, the previous image is kept. |
| `RENDERING_STABILITY_PERIOD` | `1500`                            | no       | yes      | How long (ms) the rendered DOM must stay unchanged to be considered stable. Raise it for dashboards whose cards load progressively.                                                                  |
